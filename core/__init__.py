...
# 安装顺序不可调换：
# 1. P0 记忆隔离与旧笔记工具停用；
# 2. P0.1 封锁无法解析人格/会话 scope 的 Notes Web API；
# 3. P1 安全实现替换 P0 的旧备份回灌停用入口；
# 4. P1 scoped notes 只恢复能够建立可信人格/会话 scope 的笔记链路；
# 5. P1 hardening 保证 scope 失败不打断聊天，并封住超长单行读取。
